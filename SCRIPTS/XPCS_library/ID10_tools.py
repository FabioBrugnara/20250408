...
def load_dense_e4m_new(raw_folder, sample_name, Ndataset, Nscan, Nfi, Nff,  n_jobs=6, tosparse=True, of_value=None, Nf4overflow=10):
    '''
    Load all the e4m data present in a scan.
    If tosparse=True (default) convert the dataframes to a sparse array.
    In older versions of the line ('v1') many overflow values are present in these frames, as they rapresent the lines on the detector, and also burned pixels. 
    ...
    if Nff==None: file_f = len([i for i in os.listdir(h5_folder) if i.startswith('eiger4m_v2_frame_0_')])+1 # if Nff is None, finish at the last file
    ...
    def load_framesbyfile(i, Nfi, Nff, file_i, file_f): # function to loop over files in multiprocessing
        ...
        # GET DATA
        a, b = None, None
        if file_i==i:   a = Nfi%Nfmax_dense_file
        if file_f==i+1: b = Nff%Nfmax_dense_file+1

        npA = h5file['entry_0000']['measurement']['data'][a:b]
        npA = npA.reshape((npA.shape[0], Npx))
    ...
    #### CONACATENATE THE RESULT ####
    print('Concatenating vectors ...')
    if tosparse: sA = sparse.vstack(sA)
    else:        sA = np.concatenate(sA)
    print('Done!')
